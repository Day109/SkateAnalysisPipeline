...
def Trimmomatic():
	...
	for R1 in RawDataList:
		...
		SampleId = R1.split('/')[-1].split('_')[0]

		FwRead = R1
		RvRead = R1.replace('_1.fastq.gz','_2.fastq.gz')

		PairedFw = FwRead.split('/')[-1].replace('_1.fastq.gz','_P1.fastq.gz')
		UnpairedFw = FwRead.split('/')[-1].replace('_1.fastq.gz','_U1.fastq.gz')
		PairedRv = RvRead.split('/')[-1].replace('_2.fastq.gz','_P2.fastq.gz')
		UnpairedRv = RvRead.split('/')[-1].replace('_2.fastq.gz','_U2.fastq.gz')
...
def MACS():
	...
	for BamFile in BamList:
		...
		SampleId = BamFile.split('/')[-1].replace('_MapQFilter_MtRemoved_Ind.bam','')
...
def Filter_MtDna_MapQ():
	...
	for BamFile in BamList:
		...
		#os.system('samtools view -b -q 30 %s > %s' % (BamFile,MapQFiltered))
...
if __name__ == "__main__":
	...
	Thread = 10
	ProgramDir = '/path/to/MACS_BWA_and_etcs/'

	ReferenceFile = '/path/to/reference/genome/Mus_musculus.GRCm38.dna.primary_assembly.fa'
	GffFile = '/path/to/gene/annotation/Mus_musculus.GRCm38.102.gff3'

	#RawDataList = sorted(glob.glob('AtacSeqData/MB_skate_spinalatac_*_R1_001.fastq.gz'))
	RawDataList = sorted(glob.glob('PublicData/*_1.fastq.gz'))
